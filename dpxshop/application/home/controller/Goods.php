...
//        $replyList = M('Comment')->where(['is_show'=>1,'goods_id'=>$goods_id,'parent_id'=>['>',0]])->order("add_time desc")->select();
    ...
    public function tudi(){
       ...
       return $this->fetch('goods/tudi');
    }
     public function farm_login(){
       return $this->fetch('goods/farm_login');
    }
   public function connect(){
       return $this->fetch('goods/connect');
    }
     public function farm_see(){
       return $this->fetch('goods/farm_see');
    }
    /**
     * 全部品牌列表
     * @author lxl
